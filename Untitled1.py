...
#get the view in the bird eye prespect
offset=200
...
# taking an imageand turning it into different colours spaces

row=0
sample_unwarped_image=Unwarped_images[3] 
...
for index in range(len(color_spaces)+1):
    ...
    axes[index,0].set_title(''.join(titles[index]) +' image')
    for channel in range(3):
        this_channel= colorspace[:,:,channel]
        axes[index,channel+1].imshow(this_channel,cmap='image')
        axes[index,channel+1].set_title(titles[index][channel]+' image')
